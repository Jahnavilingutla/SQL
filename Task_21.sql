...
Q1)From the following table, create a view for those salespeople who
belong to the city of New York.

A)create view Newyork
as
select * from SALESMAN where city='New York'

select * from Newyork

Q)2. From the following table, create a view for all salespersons. Return
salesperson ID, name, and city.

A)create view salespersons
as
...
Q)3. From the following table, create a view to count the number of
salespeople in each city. Return city, number of salespersons.

A)create view count_staff
as
...
4.Q) From the following table, create a view that counts the number of
customers in each grade.

A)create view customer_count
as
select grade, count(customer_id) as customers_count from Customer group by grade
...
Q5).create a view to get the salesperson and customer by name. Return order name, purchase amount, salesperson ID, name, customer name.

A)create view Order_detail
  as
